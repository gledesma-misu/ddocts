...
class Dashboard extends CI_Controller
{
    ...
    public function recieve_doc()
    {
        ...
        $single_doc = $this->dashboard->get_single_doc($get_doc);
        $concern_staff = $single_doc['dd_routed_to'];
        $check_division = $this->session->userdata('staff_division');;

        $this->dashboard->recieve_docs($get_doc, $name_doc, $name, $concern_staff, $check_division);
    ...
    public function getNotif()
    {
        ...
        if ($this->input->post("view") != '') {
            ...
            $this->db->query($sql);
            // $this->dashboard->read_notification($staff);
        }
        $output = '';
        if ($notif->num_rows() > 0) {
            foreach ($notif->result_array() as $row) {
                ...
                $output .= '<div class="cursor-pointer relative flex items-center mt-5 notification-item" data-id="' . $row["id"] . '">
    ...
    public function readNotif()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($notificationId) {
                $response = array('status' => 'success', 'message' => 'Notification marked as read.');
            }
            log_message('debug', 'Data Files: ' . print_r($this->input->post("notification_id"), true));

            $this->dashboard->read_notification($staff, $notificationId);
